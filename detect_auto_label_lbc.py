...
def auto_label(v7pose_txt_folder, output_json):
        ...
        if file.endswith(".txt"):
            ...
            with open(os.path.join(v7pose_txt_folder, file), "r", encoding="utf-8") as f:
                ...
                for content in contents:
                    ...
                    elements_float = [float(ele) for ele in elements]
                    bbox_float = elements_float[1:5]
                    bbox_float[2] = bbox_float[2]-bbox_float[0]
                    bbox_float[3] = bbox_float[3]-bbox_float[1]
                    kps_value = elements_float[5:56]
                    kps_float = [2 if (index + 1) % 3 == 0 else x for index, x in enumerate(kps_value)]
                    img_width = int(elements_float[56])
                    img_height = int(elements_float[57])
                    if idx == 0:
...
def detect(opt):
    source, weights, view_img, save_txt, imgsz, save_txt_tidl, kpt_label, save_lbc_txt = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.save_txt_tidl, opt.kpt_label, opt.save_lbc_txt
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(det[:,:6]):
                    if save_txt:  # Write to file
                        ...
                        if opt.save_lbc_txt:
                            wh = gn.tolist()[:2]
                            kpts_txt = det[det_index, 6:].tolist()
                            xyxy = [round(x.item(), 2) for x in xyxy]
                            line = (cls, *xyxy, *kpts_txt, *wh)
                        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-lbc-txt', action='store_true', help='save results to *.txt to generate auto label for LBC in --save-txt labels')
