...
class CourtDetector:
  ...
  def visualize(self, img, lines):
      for i in range(0, len(lines), 4):
          x1, y1, x2, y2 = lines[i], lines[i + 1], lines[i + 2], lines[i + 3]
          cv2.line(img, (int(x1), int(y1)), (int(x2), int(y2)), (128, 0, 0), 5)

  def begin(self, type="detect", **kwargs):
      if type == "detect":
          detect_fn = self.detect
      else:
  ...
  def detect(self, frame, verbose=0, mask_points=[]):
    ...
    if os.path.exists(self.param_path):
        court_warp_matrix, game_warp_matrix, self.court_score, self.best_conf = \
            np.load("{}/court_warp_matrix.npz.npy".format(self.param_path)), \
            np.load("{}/game_warp_matrix.npz.npy".format(self.param_path)), \
            np.load("{}/court_score.npz.npy".format(self.param_path)), \
            np.load("{}/best_conf.npz.npy".format(self.param_path)).tolist()
    else:
        ...
        if self.param_path:
            ...
            np.save("{}/court_warp_matrix.npz.npy".format(self.param_path), court_warp_matrix)
            np.save("{}/game_warp_matrix.npz.npy".format(self.param_path), game_warp_matrix)
            np.save("{}/court_score.npz.npy".format(self.param_path), self.court_score)
            np.save("{}/best_conf.npz.npy".format(self.param_path), self.best_conf)
  ...
  def detect_court_with_inner_points(self, frame, mask_points, **kwargs):
      ...
      self.game_warp_matrix.append(cv2.invert(matrix)[1])
      self.best_conf = 1
      # cv2.imshow("hdasjk", transformed_image)
      # cv2.waitKey(0)
      return self.find_lines_location()
