...
def detect(opt):
    ...
    for idx, (path, img, im0s, vid_cap) in enumerate(dataset):
        frame_content = dict()
        frame_content["frame"] = idx
        frame_content["objects"] = []
        frame_content["img_height"] = im0s.shape[0]
        frame_content["img_width"] = im0s.shape[1]
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if json_path:
                        tmp = dict()
                        tmp["bbox"] = [int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])]
                        tmp["score"] = float(conf)
                        tmp["kps"] = det[det_index, 6:].tolist()
                        tmp["idx"] = det_index
                        frame_content["objects"].append(tmp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json-path', default="", help='save results to *.txt')
