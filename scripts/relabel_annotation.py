    ...
    for root, dirs, files in os.walk(input_csv_folder):
        for file in files:
            if file.endswith('.txt'):
                ...
                video_file_path = os.path.join(input_video_folder, file.replace('.txt', '.mp4'))
...
def read_csv_file(txt_file, frame_width, frame_height):

    with open(txt_file, 'r') as f:
        lines = f.readlines()
    ball_locations, frame_ls, curve_predictions = [], [], []
    for idx, line in enumerate(lines):
        line = line.strip()
        if line:
            x, y, status = line.split(',')
            ball_locations.append([float(x)*frame_width, float(y)*frame_height])
            frame_ls.append(idx)
            curve_predictions.append(int(status))

    print(f'first_frame:{frame_ls[0]}, last_frame:{frame_ls[-1]}, len(frame_ls):{len(frame_ls)}')
    return ball_locations, frame_ls, curve_predictions
...
def main(csv_path, video_path, output_txt_folder):
    ...
    ball_locations, frame_ls, curve_predictions = read_csv_file(csv_path, frame_width, frame_height)
