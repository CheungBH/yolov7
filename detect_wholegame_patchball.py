...
import shutil
from detect_patch import to_original_coord, remove_boundary_boxes, post_nms, patch_crop
        ...
        if classifier_status == 0:
            ...
            if use_saved_box:
                ...
                with torch.no_grad():  # Calculating gradients would cause a GPU memory leak
                    ball_pred = ball_model(batch_imgs, augment=opt.augment)[0]
                    pose_pred = pose_model(img, augment=opt.augment)[0]
                t2 = time_synchronized()

                # Apply NMS
                final_preds = torch.zeros(1, 6)
                ball_pred = non_max_suppression(ball_pred, opt.ball_thres, opt.iou_thres, classes=opt.classes,
                                                agnostic=opt.agnostic_nms)
                scaled_coords = []
                for p, p_img, tl, im in zip(ball_pred, patches, tls, batch_imgs):
                ...
                ball_pred = [post_nms(final_preds[1:], 0.8)]

                pose_pred = pose_non_max_suppression(pose_pred, opt.pose_thres, opt.iou_thres, classes=opt.classes,
                                                     agnostic=opt.agnostic_nms, kpt_label=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_folder", default=r"D:\tmp\4.8\results\whole\game1")
